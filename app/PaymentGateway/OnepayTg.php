...
class OnepayTg implements PaymentInterface
{
    const NAME = 'onepaytg';
    ...
    private function buildUrl($data)
    {
        ...
        return env('CALLBACK_SERVER_TG') . '?' . implode("&", $flatdata);
    }

    public function checkHash($input)
    {
        if (
            strlen(env('PAYMENT_ONEPAY_TG_SECRET')) > 0
            ...
            $checkedHash = strtoupper(hash_hmac('SHA256', $stringHashData, pack('H*', env('PAYMENT_ONEPAY_TG_SECRET'))));
    ...
    {
        $returnUrl = env('APP_URL') . '/payment-return/onepaytg';
        $notifyurl = env('APP_URL') . '/payment-notify/onepaytg';
        $data =  [
            'vpc_Version' => 2,
            'vpc_Currency' => 'VND',
            'vpc_Command' => 'pay',
            'vpc_AccessCode' => env('PAYMENT_ONEPAY_TG_ACCESSCODE'),
            'vpc_Merchant' => env('PAYMENT_ONEPAY_TG_MERCHANT'),
            'vpc_Locale' => 'vn',
        ...
        $signature =  strtoupper(hash_hmac('sha256', $hashRaw, pack('H*', env('PAYMENT_ONEPAY_TG_SECRET'))));
    ...
    private function getServer()
    {
        ...
        return env('PAYMENT_ONEPAY_TG_SERVER', 'https://mtf.onepay.vn/paygate/vpcpay.op');
