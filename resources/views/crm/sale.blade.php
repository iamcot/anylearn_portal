...
<div class="row crm mb-5">
    <div class="col-lg-6">
        ...
        <div class="card shadow mt-3">
            ...
            <div class="card-body">
                <dl class="row">
                    ...
                    <dt class="col-sm-8">{{ $memberProfile->phone }} <a class="ml-2 phonecall" href="#" data-phone="{{ $memberProfile->phone }}" title="{{ $memberProfile->phone }}"><i class="fa fa-phone"></i></a></dt>
                    <dd class="col-sm-3">Email</dd>
                    <dt class="col-sm-8">{{ $memberProfile->email }} <a class="ml-2" href="mailto:{{ $memberProfile->email }}"><i class="fa fa-envelope"></i></a></dt>
                </dl>
            </div>
        </div>
        <form action="{{ route('crm.save-note') }}" method="POST">
            ...
            <div class="card shadow mt-3">
                <div class="card-header"><strong>Ghi chú</strong> <i>(Cập nhật cuối: {{ $lastNote ? date('H:i d/m/y', strtotime($lastNote->created_at)) : '' }})</i> <button name="action" value="save-note" class="btn btn-success btn-sm float-right"><i class="fa fa-save"></i></button></div>
    ...
    <div class="col-lg-6">
        <div class="card shadow">
            <div class="card-header"><strong>Lịch sử liên hệ</strong>
                <a id="add-chat" class="btn btn-success btn-sm float-right ml-2" title="Thêm chat mới"><i class="fas fa-comment-medical text-white"></i></a>
                <a id="add-call" class="btn btn-success btn-sm float-right" title="Thêm call mới"><i class="fas fa-phone text-white"></i></a>
            </div>
            <div class="card-body p-0">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <a href="{{ route('crm.activity.del', ['id' => $contact->id]) }}"><i class="fa fa-trash text-danger"></i></a>
                                {{ date('H:i d/m/y', strtotime($contact->created_at)) }}
                            </td>
                            <td><i class="fa fa-{{ $contact->type == 'call' ? 'phone' : 'comment'}}"></i></td>
                            <td class="text-center">{{ $contact->logwork }}</td>
                            <td>
                                <a href="#" class="btn btn-warning btn-sm activity-content-view" data-id="{{ $contact->id }}">XEM</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
        ...
        <div class="card shadow mt-3">
            <div class="card-header"><strong>Lịch sử đặt hàng</strong></div>
            <div class="card-body p-0">
                <table class="table">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $item->order_id }}</td>
                            <td><a target="_blank" href="{{ $itemServ->classUrl($item->itemId) }}">{{ $item->title }}</a></td>
            ...
            callbacks: {
                ...
                ended: (data) => {
                    ...
                    if (data.isAccepted && data.duration > 0) {
                        $.ajax({
                            url: "{{ route('crm.save-call') }}" + "?action=save-call&member_id={{ $memberProfile->id }}&isajax=1&logwork=" + data.duration + "&uuid=" + data.uuid,
